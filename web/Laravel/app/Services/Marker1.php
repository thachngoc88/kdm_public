...
class Marker1
{
    ...
    public static function mark($force = false){
        ...
        Log::info("Before run Marker1..................");

        if($force || (isset($ml) && isset($record) && ($record->updated_at > $ml->updated_at))){
            echo("Begin execute maker 1 !!!");
            Log::info("Begin execute maker 1 !!!");
            try {
                $log = new MarkingLog();
                $log->save();
...
//            throw new \Exception('Fail to mark ' . $e->getMessage() . ' ');
            ...
            Log::info("Handling mark 1 is done. Time(sec): " . $timeStart->diffInSeconds(Carbon::now()));
        } else {
