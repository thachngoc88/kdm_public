...
class Marker13
{
    ...
    public static function mark($force = false){
        ...
        Log::info("Before run Marker 1.3 ..................");
        ...
        if($force || (isset($ml) && isset($record) && ($record->updated_at > $ml->updated_at))){
            echo("Begin execute maker 1.3!!!");
            Log::info("Begin execute maker 1.3 !!!");
            try {
                $log = new MarkingLog();
                $log->save();
...
//            $this->updateMarks($log);
                ...
                $mode = AggregationDataTable::MODE_CITIES;
...
//            throw new \Exception('Fail to mark ' . $e->getMessage() . ' ');
            ...
            Log::info("Handling mark 1.3 is done. Time(sec): " . $timeStart->diffInSeconds(Carbon::now()));
        } else {
            echo("Not execute maker 1.3!!!");
            Log::info("Not execute maker 1.3!!!");
        }
