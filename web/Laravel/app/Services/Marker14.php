...
class Marker14
{
    ...
    public static function mark($force = false){
        ...
        Log::info("Before run Marker 1.4 ..................");
        ...
        if($force || (isset($ml) && isset($record) && ($record->updated_at > $ml->updated_at))){
            echo("Begin execute maker 1.4!!!");
            Log::info("Begin execute maker 1.4 !!!");
            try {
                $log = new MarkingLog();
                $log->save();
...
//            $this->updateMarks($log);
                ...
                $mode = AggregationDataTable::MODE_PREFECTURES;

                PassingRateUpdater::updatePassingRatesByMode($log, $mode);
                AggregationUpdater::updateAggregationCurriculumsByMode($log, $mode);
...
//            throw new \Exception('Fail to mark ' . $e->getMessage() . ' ');
            ...
            Log::info("Handling mark 1.4 is done. Time(sec): " . $timeStart->diffInSeconds(Carbon::now()));
        } else {
            echo("Not execute maker 1.4!!!");
            Log::info("Not execute maker 1.4!!!");
        }
