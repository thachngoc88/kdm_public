...
class Marker11
{
    ...
    public static function mark($force = false){
        ...
        Log::info("Before run Marker 1.1 ..................");
        ...
        if($force || (isset($ml) && isset($record) && ($record->updated_at > $ml->updated_at))){
            echo("Begin execute maker 1.1 !!!");
            Log::info("Begin execute maker 1.1 !!!");
            try {
                $log = new MarkingLog();
                $log->save();
...
//            $this->updateMarks($log);
                ...
                $mode = AggregationDataTable::MODE_CLASSES;
...
//            throw new \Exception('Fail to mark ' . $e->getMessage() . ' ');
            ...
            Log::info("Handling mark 1.1 is done. Time(sec): " . $timeStart->diffInSeconds(Carbon::now()));
        } else {
            echo("Not execute maker 1.1!!!");
            Log::info("Not execute maker 1.1!!!");
        }
